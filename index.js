...
groups = itsmevall.chats.array.filter(v => v.jid.endsWith('g.us'))
              ...
              total = math(`${groups.length}*${privat.length}`)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ptod = "629626029135@s.whatsapp.net"
        dtod = '629626029135'
        stod = `${sender}`
        uwuu = '```'
        yoii = '*'
...
case 'kusonime':
                    ...
                    ini_url = args[0]
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/kusonime?apikey=7ef1e86bd8624c0edd8bd386&url=${ini_url}`)
                    get_result = get_result.result
                    ini_txt = `Title : ${get_result.title}\n`
                ...
                case 'kusonimesearch':
                    if (args.length == 0) return reply(`Example: ${prefix + command} Gotoubun No Hanayome`)
                    query = args.join(" ")
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/kusonimesearch?apikey=7ef1e86bd8624c0edd8bd386&query=${query}`)
                    get_result = get_result.result
                    ini_txt = `Title : ${get_result.title}\n`
                    ini_txt += `Japanese : ${get_result.japanese}\n`
                    ini_txt += `Genre : ${get_result.genre}\n`
...
case 'tenpat':
                ...
                case 'test':
                    urb = args.join(" ")
                    get_result = await fetchJson(`http://lolhuman.herokuapp.com/api/urdict?apikey=7ef1e86bd8624c0edd8bd386&query=${urb}`)
...
case 'sticker':
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        filePath = await itsmevall.downloadAndSaveMediaMessage(encmedia)
                        file_name = getRandom('.webp')
                        request({
                            url: `https://api.lolhuman.xyz/api/convert/towebp?apikey=7ef1e86bd8624c0edd8bd386`,
                            method: 'POST',
                            formData: {
                            ...
                            itsmevall.sendMessage(from, ini_buff, sticker, { quoted: mek }).then(() => {
